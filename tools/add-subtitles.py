...
def get_lang_code(filename):
    ...
    # 1. Strip extension to avoid matching 'ass' or 'srt' accidentally (though unlikely with this map)
    # Actually, we keep it just in case user names it "Show.fr.ass"
...
def find_mkvmerge():
    """Finds mkvmerge in the portable tools folder."""
    # Since the script is running from the root (via the .bat cd ..), we look in tools/
    path = os.path.join("tools", "MKVToolNix", "mkvmerge.exe")
    if os.path.exists(path):
        return path
    print(f"[ERROR] Could not find mkvmerge at: {path}")
    sys.exit(1)
...
def process_files(mkvmerge, mkv_input, subtitle_files, font_files):
    ...
    # print("DEBUG COMMAND:", " ".join(cmd)) # Uncomment to see exact command
...
def main():
    ...
    if len(sys.argv) > 1:
        # Strip quotes just in case the batch file passed them weirdly
    ...
    subs = glob.glob(os.path.join(target_dir, "*.ass")) + glob.glob(os.path.join(target_dir, "*.srt"))
    fonts = glob.glob(os.path.join(target_dir, "*.ttf")) + glob.glob(os.path.join(target_dir, "*.otf"))
    ...
    if not mkvs:
        ...
        print("Ensure your .mkv files are in the same folder as the .bat file.")
        return
        ...
        for mkv_file in mkvs:
            ...
            if match:
                ...
                if current_subs or fonts:
                    ...
                    print(f"    [Skipping] No matching subtitles or fonts found for {basename}")
            else:
