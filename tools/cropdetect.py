...
VIDEO_DEFAULT_EXTS = {
    ".mp4", ".mkv", ".mov", ".m4v", ".webm", ".avi", ".mpg", ".mpeg", ".ts", ".m2ts", ".wmv"
}
...
def setup_temp_dir():
    ...
    if temp_path.exists():
        try:
            ...
            print(f"Warning: Could not delete old temp dir {temp_path}: {e}", file=sys.stderr)
...
def run_cropdetect_segment(video: Path, ss: float, seg: float, fps: float, limit: float, round_to: int) -> List[Tuple[int, int, int, int]]:
    vf = f"fps={fps},format=yuv444p,cropdetect=limit={limit}:round={round_to}:reset=0"
    cmd = [
        "ffmpeg", "-hide_banner", "-loglevel", "info",
        "-ss", f"{ss:.3f}", "-i", str(video), "-t", f"{seg:.3f}",
        "-vf", vf, "-an", "-sn", "-dn", "-f", "null", "-"
    ]
...
def choose_best_crop(vi: VideoInfo, observed: Counter, crop_to_limits: Dict[str, set]) -> Optional[CropResult]:
    if not observed:
        return None
    ...
    for crop_str, count in observed.items():
        ...
        a = area(w, h)
        if a <= 0 or a > full_area: continue
    ...
    notes = "strong" if lim_support >= 3 else "moderate" if lim_support == 2 else "single-threshold"
    return CropResult(crop_str, w, h, x, y, freq, total, [float(v) for v in limits], f"{notes} agreement")

def detect_ffmpeg(vi: VideoInfo, sample_count: int, segment_len: float, fps: float, limits: List[float], round_to: int, progress_mode: bool) -> Optional[CropResult]:
...
def detect_vapoursynth(vi: VideoInfo, mode: int, value: int, lum_thresh_int: int, progress_mode: bool) -> Optional[CropResult]:
    if not VS_AVAILABLE:
...
"""
    ...
    try:
        ...
        if mode == 1: interval = considered // value
        elif mode == 2: interval = value
        elif mode == 3: interval = int(value * fps)
        ...
        return CropResult(
            crop=crop_str, w=final_w, h=final_h, x=min_left, y=min_top,
            confidence=1.0, samples=len(analyze_frames),
            chosen_from_limits=[], notes="VapourSynth Aggressive Scan"
        )
...
def main() -> int:
    ...
    # 2. Parse Args
    ap = argparse.ArgumentParser(description="Robust Crop Detect (FFmpeg & VapourSynth)")
    ap.add_argument("inputs", nargs="+", help="Video files/dirs")
    ap.add_argument("--recursive", action="store_true")
    ap.add_argument("--extensions", default=",".join(sorted(e.lstrip(".") for e in VIDEO_DEFAULT_EXTS)))
    ap.add_argument("--samples", type=int, default=15, help="FFmpeg mode samples")
    ap.add_argument("--segment", type=float, default=2.5, help="FFmpeg mode segment seconds")
    ap.add_argument("--fps", type=float, default=3.0)
    ap.add_argument("--round", dest="round_to", type=int, default=2)
    ap.add_argument("--aggressive", action="store_true", help="Use VapourSynth+NumPy (Slower, precise)")
    ...
    if crop_mode == "manual":
        for k in manual_crop:
            try: manual_crop[k] = int(settings.get(k, 0))
            except: pass
    ...
    for idx, vp in enumerate(videos, 1):
        ...
        if crop_mode == "manual":
            ...
            if mw <= 0 or mh <= 0: mw, mh = vi.width, vi.height
            res = CropResult(f"{mw}:{mh}:{manual_crop['left']}:{manual_crop['top']}",
                             mw, mh, manual_crop["left"], manual_crop["top"], 1.0, 1, [], "Manual")
            if args.progress_mode: print("PROGRESS:100", flush=True)

        elif args.aggressive:
            # --- VapourSynth Mode ---
            res = detect_vapoursynth(vi, mode=3, value=15, lum_thresh_int=1000, progress_mode=args.progress_mode)
            
        else:
            # --- FFmpeg Mode ---
            limits = [0.06, 0.08, 0.12, 0.18, 0.25, 0.35]
            res = detect_ffmpeg(vi, args.samples, args.segment, args.fps, limits, args.round_to, args.progress_mode)

        # Fallback
        if not res:
            res = CropResult(f"{vi.width}:{vi.height}:0:0", vi.width, vi.height, 0, 0, 0.0, 0, [], "Failed/Full")

        row = {
            "file": str(vp),
            "width": vi.width, "height": vi.height, "duration_sec": round(vi.duration, 3),
            "crop": res.crop, "crop_w": res.w, "crop_h": res.h, "crop_x": res.x, "crop_y": res.y,
            "confidence": round(res.confidence, 4), "samples_seen": res.samples,
    ...
    keys = ["file", "width", "height", "duration_sec", "crop", "crop_w", "crop_h", "crop_x", "crop_y",
    ...
    if args.json_out:
        Path(args.json_out).write_text(json.dumps(json_results, indent=2), encoding="utf-8")
