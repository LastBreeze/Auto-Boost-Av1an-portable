...
def resolve_tool(portable_path_str: str, binary_name: str) -> Path:
    ...
    """
    # 1. Check Portable Path
    p_path = Path(portable_path_str).resolve()
    if p_path.exists():
        return p_path
        ...
        if not fallback_needed:
            ...
            try:
                ...
                if temp_json.exists():
                    ...
                    raise RuntimeError("JSON output not generated by FFVship.")
            except Exception as e:
                if ssimu2 == 'auto':
                    console.print(f"[yellow]FFVship failed: {e}. Fallback triggered.[/yellow]")
                    fallback_needed = True
                else:
